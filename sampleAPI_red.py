...
def run_calibration(webcam, gaze, calibrator):
    ...
    for i in range(len(calibrator.matrix.points)):
        ...
        while time.time() - start_time < conf.POINT_DISPLAY_TIME:
            ...
            cv2.putText(dot_screen, text, (80, 100), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2)
...
def run_gaze_api(webcam, gaze, calibrator):
    ...
    # --- [추가] 시각화 창 이름 설정 ---
    debug_window_name = "Gaze Tracking Visualizer"
    ...
    while True:
        ...
        if gaze.pupils_located:
            ...
            # --- [추가] 시각화용 디버그 창 생성 및 업데이트 ---
            # 1. 검은색 배경 생성
            debug_screen = np.zeros((conf.LOGICAL_HEIGHT, conf.LOGICAL_WIDTH, 3), dtype=np.uint8)
            # 2. 현재 시선 좌표에 빨간색 원 그리기
            cv2.circle(debug_screen, last_stable_screen, 30, (0, 0, 255), -1)
            # 3. 디버그 창 보여주기
            cv2.imshow(debug_window_name, debug_screen)
            # --- 시각화 코드 끝 ---
