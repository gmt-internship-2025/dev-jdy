...
# ===== 노이즈 억제 파라미터 =====
ema_alpha = 0.35          # EMA 강도 (0.2~0.5 권장)
DEADZONE_PX = 25          # 이 픽셀 이내 움직임은 무시
SMOOTH_STEP_PX = 40       # 프레임당 최대 이동 픽셀 (slew limit)
...
if os.path.exists("calibration_data_1.pkl"):
    ...
    print("calibration_data_1.pkl not found.")
    raise SystemExit(1)
...
# ===== 스무딩 상태 =====
ema_pred = None                  # EMA된 예측 (norm)
last_stable_screen = None        # 데드존/슬루 제한이 적용된 "확정 출력" 좌표 (px)

# ===== HTTP 전송 최적화 =====
session = requests.Session()
POST_URL = "http://localhost:5000/api/gaze"
POST_TIMEOUT = 0.08
POST_HZ = 60.0
