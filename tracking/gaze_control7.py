...
if os.path.exists(conf.CALIBRATION_FILE):
    ...
    print(f"[!] {conf.CALIBRATION_FILE} 파일이 없습니다.")
    raise SystemExit(1)
...
def clamp_to_screen(pt):
    # conf.py의 화면 해상도 사용
    x = max(0, min(conf.SCREEN_WIDTH - 1, int(pt[0])))
    y = max(0, min(conf.SCREEN_HEIGHT - 1, int(pt[1])))
    return (x, y)
...
while True:
    ...
    if left_pupil and right_pupil:
        ...
        # 원하는 위치(EMA 적용값)를 픽셀로 (conf.py의 화면 해상도 사용)
        desired_screen = (ema_pred[0] * conf.SCREEN_WIDTH, ema_pred[1] * conf.SCREEN_HEIGHT)
